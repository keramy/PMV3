...
export const PATCH = withAuth(async (user, request) => {
  ...
  // Enhanced admin permission check - look for admin-related permissions
...
export const DELETE = withAuth(async (user, request) => {
  ...
  if (!hasAdminAccess) {
    console.log('🚫 [Admin API] Deactivation denied for user:', user.id, 'permissions:', permissions)
    return ApiResponses.forbidden('Admin access required - need admin_access or manage_users permission for user deactivation')
  }
