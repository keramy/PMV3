...
interface MaterialSpecsTableProps {
  projectId?: string
}

export function MaterialSpecsTable({ projectId }: MaterialSpecsTableProps) {
  ...
  // Fetch material specs from API
  const { data: specsResponse, isLoading, error } = useMaterialSpecs({
    project_id: projectId || 'all', // Use passed projectId or 'all'
    page: 1,
    limit: 50,
    category: searchCategory === 'all' ? undefined : searchCategory as any,
    search_term: searchQuery || undefined,
  })

  // Mutation hooks for API operations
  const createMaterialSpec = useCreateMaterialSpec()
  const updateMaterialSpec = useUpdateMaterialSpec()
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-2 text-gray-600">Loading material specs...</span>
      </div>
    )
  }
  ...
  if (error) {
      ...
      return (
        <div className="p-6">
          <EmptyState
            ...
            description="Please log in to access material specifications."
            action={{
              label: "Go to Login",
      ...
      return (
        <div className="p-6">
          <EmptyState
            icon={Package}
            title="Access Denied"
            description="You don't have permission to view material specifications. Contact your administrator."
          />
        </div>
      )
    }
    ...
    return (
      <div className="p-6">
        <EmptyState
          icon={Package}
          title="Error Loading Material Specs"
          description={`Failed to load material specifications: ${errorMessage}`}
          action={{
            label: "Retry",
  ...
  const exportToExcel = () => {
    ...
    const exportData = specs.map(spec => ({
      ...
      'Project': spec.project?.name || 'N/A',
      'Status': getStatusLabel(spec.status),
      'Manufacturer': spec.manufacturer || '',
      'Specification': spec.specification || '',
      'Notes': spec.notes || '',
      'Submitted By': spec.created_by_user ? `${spec.created_by_user.first_name} ${spec.created_by_user.last_name}` : 'N/A',
      'Submitted Date': formatDate(spec.created_at)
    }))
                  ...
                  <Button 
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (isEditMode && editingSpec?.id) {
                          await updateMaterialSpec.mutateAsync({
                            id: editingSpec.id,
                            data: materialData
                          })
                        } else {
                          await createMaterialSpec.mutateAsync(materialData)
                        }

                        setIsAddModalOpen(false)
                        resetForm()
                      } catch (error) {
                        console.error('Failed to save material spec:', error)
      ...
      <div className="rounded-xl border shadow-md bg-white overflow-hidden">
        ...
        {specs.length === 0 && (
          <div className="py-12">
            <EmptyState
              icon={Package}
              title="No Material Specifications"
              description="No material specifications found. Create your first spec to get started."
              action={{
                label: "Add Material Spec",
