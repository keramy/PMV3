-- Migration: update_material_categories_fixed
-- Updates material categories to new simplified system (CORRECTED)
...
-- =============================================
-- 4. LOG MIGRATION COMPLETION (CORRECTED)
-- =============================================

-- Insert completion log using correct activity_logs schema
DO $$
BEGIN
    ...
    IF EXISTS (
        ...
        SELECT 
            ...
            'material_specs' as entity_type,
            '{"description": "Updated material categories to simplified material-based system (wood, metal, glass, etc.)", "categories_updated": ["wood", "metal", "glass", "stone", "paint", "floor", "fabric", "hardware", "miscellaneous"]}'::jsonb as details,
            NOW() as created_at
        FROM user_profiles 
        WHERE 'admin' = ANY(permissions) OR 'approve_material_specs' = ANY(permissions)
        LIMIT 1;
